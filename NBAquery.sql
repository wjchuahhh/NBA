...
select Player, t1.Tm, PTS, Win, Loss, cast(Win as int)+cast(Loss as int) as TotalGames, round(cast(Win as float)/(cast(Win as float)+cast(Loss as float))*100,2) as WinPercentage,t1.Season
from NBA..PerGameStats as t1
join NBA..Standings as t2
on t1.Tm = t2.tm and t1.Season = t2.Season
where PTS > 20
order by PTS desc

-- Effect of best passing player of the team on the team's win-loss record

select Player, t1.Tm, AST, Win, Loss, cast(Win as int)+cast(Loss as int) as TotalGames, round(cast(Win as float)/(cast(Win as float)+cast(Loss as float))*100,2) as WinPercentage,t1.Season
from NBA..PerGameStats as t1
join NBA..Standings as t2
on t1.Tm = t2.tm and t1.Season = t2.Season
where AST > 6
